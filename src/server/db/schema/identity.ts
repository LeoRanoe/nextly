...
export const settings = pgTable(
  'settings',
  {
    ...
    pickupLabel: text(),
    pickupDetails: text(),
    sameDayPickupEnabled: boolean().notNull().default(false),
    pickupCutoffTime: text(),
    deliveryEnabled: boolean().notNull().default(false),
    deliveryDetails: text(),
    deliveryAreas: text(),
    deliveryFeeDisplay: text(),
    deliveryEstimateDisplay: text(),
    paymentMethods: jsonb().$type<{ name: string; details?: string }[]>().notNull().default([]),
    announcement: text(),
    heroTitle: text(),
    heroBody: text(),
    supportTitle: text(),
    supportBody: text(),
    defaultNewArrivalDays: bigint({ mode: 'number' }).notNull().default(30),
    homepageAvailableNowEnabled: boolean().notNull().default(true),
    homepageAvailableNowLimit: integer().notNull().default(6),
    homepageFeaturedBundlesEnabled: boolean().notNull().default(true),
    homepageFeaturedBundlesLimit: integer().notNull().default(3),
    homepageJustArrivedEnabled: boolean().notNull().default(true),
    homepageJustArrivedLimit: integer().notNull().default(4),
    homepageComingNextEnabled: boolean().notNull().default(true),
    homepageComingNextLimit: integer().notNull().default(4),
