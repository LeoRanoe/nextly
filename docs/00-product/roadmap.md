...
| **Public catalog** | `/catalog` and `/catalog/[slug]` in their own route group: no auth guard, no cost figure selected anywhere, everything filtered on `catalog_published AND status = 'active'` by the queries themselves. Same rows, same ledger-derived availability the dashboard sees — the two cannot disagree |
